...
export default function AuthModal({ isOpen, onClose, onAuthSuccess, initialMode = "login" }: AuthModalProps) {
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.error || "Không tìm thấy email đăng ký.");
      }
      ...
      <div id="auth-modal-card" className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden border border-gray-100 flex flex-col relative animate-fade-in">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-8 text-white relative">
          <button
            type="button"
            onClick={onClose}
            className="absolute top-4 right-4 text-white/80 hover:text-white p-1 rounded-full bg-black/10 hover:bg-black/20 transition-all cursor-pointer"
          >
            <X className="h-5 w-5" />
          </button>
          ...
          {mode === "login" && (
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full text-xs bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 text-white font-bold py-3 px-4 rounded-xl shadow-lg shadow-blue-500/10 cursor-pointer flex items-center justify-center gap-1.5 transition-all"
              >
          ...
          {mode === "forgot" && (
            <form onSubmit={handleForgotPassword} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full text-xs bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 text-white font-bold py-3.5 px-4 rounded-xl cursor-pointer transition-all flex items-center justify-center gap-1.5"
              >
          ...
          {mode === "reset" && (
            <form onSubmit={handleResetPassword} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full text-xs bg-emerald-600 hover:bg-emerald-700 disabled:bg-gray-400 text-white font-bold py-3 px-4 rounded-xl shadow-lg cursor-pointer transition-all"
              >
              ...
              <div className="pt-2 text-center text-xs">
                <button
                  ...
                  className="text-slate-500 hover:text-slate-800 font-extrabold cursor-pointer"
                >
